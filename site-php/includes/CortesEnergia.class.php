<?php
    ...
    class CortesEnergia{
        public static function create_corteEnergia($id_planta, $id_cliente, $fecha, $observacion,$estado, $id_usuario){
            $database = new Database();
            $conn = $database->getConnection();
            $stmt = $conn->prepare('INSERT INTO cctv_reporte_corte_energia (id_planta,id_cliente,fecha,observacion,estado, id_usuario)
        ...
        public static function update_corteEnergia($id, $fecha, $observacion,$estado){
            $database = new Database();
            $conn = $database->getConnection();

            $stmt = $conn->prepare('UPDATE cctv_reporte_corte_energia SET fecha=:fecha, observacion=:observacion, estado=:estado WHERE id=:id');
            $stmt->bindParam(':fecha',$fecha);
            $stmt->bindParam(':observacion',$observacion);
            $stmt->bindParam(':estado',$estado);
            $stmt->bindParam(':id',$id);
